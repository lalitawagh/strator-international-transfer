                ...
                if (\Illuminate\Support\Facades\Auth::user()->isSuperadmin()){
                    if ($model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::CANCELLED){
                        if ($model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::COMPLETED){
                            $actions[] = ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" icon-name="check-circle" data-lucide="check-circle" class="lucide lucide-check-circle w-4 h-4 mr-2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>','isOverlay' => '0','route' => route('dashboard.international-transfer.money-transfer.transferCompleted', $value),'action' => 'Completed'];
                        ...
                        if ($model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::ACCEPTED &&
                            ...
                            $actions[] = ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" icon-name="check" data-lucide="check" class="lucide lucide-check w-4 h-4 mr-2"><polyline points="20 6 9 17 4 12"></polyline></svg>','isOverlay' => '0','route' => route('dashboard.international-transfer.money-transfer.transferAccepted', $value),'action' => 'Accepted'];

                        }
                        if ($model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::ACCEPTED &&
                            $model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::PENDING &&
                            $model->status != \Kanexy\PartnerFoundation\Core\Enums\TransactionStatus::COMPLETED){
                                $actions[] = ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" icon-name="alert-circle" data-lucide="alert-circle" class="lucide lucide-alert-circle w-4 h-4 mr-2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>','isOverlay' => '0','route' => route('dashboard.international-transfer.money-transfer.transferPending', $value),'action' => 'Pending'];
                        }
                    }
                }
