<div wire:key="tr_log_id_{{ $transaction->id }}">
    <div class="py-5">
        @if ($logSent == true)
    ...
    <div class="grid grid-cols-12 gap-0" style="max-height:350px;overflow-y:auto;"
        wire:key="tr_info_{{ $transaction->id }}">
            ...
            @if (!isset($transactionLogs))
                <div class="mt-5">
                    <svg width="25" viewBox="-2 -2 42 42" xmlns="http://www.w3.org/2000/svg"
                        stroke="rgb(45, 55, 72)" class="w-8 h-8 block mx-auto">
            ...
            @else
                @foreach ($transactionLogs as $index => $log)
                    <div class="report-timeline mb-0 relative"
                        wire:key="tr_{{ $index }}_logs_{{ $log->created_at }}">
                        <div class="intro-x relative flex items-center pb-3">
                            ...
                            <div class="box px-5 py-3 ml-4 flex-1 zoom-in">
                                <div class="flex items-center">
                                    ...
                                    <div class="text-xs text-gray-500 ml-auto"> {{ $log?->user?->getFullName() }},
                                        {{ date('D d/m/Y H:i', strtotime($log->updated_at)) }}</div>
                                </div>
